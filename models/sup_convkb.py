...
OUTPUT_DIR   = ROOT / "outputs" / "convkb_sup_output"             # folder for saved outputs
...
EMBEDDING_DIM = 100        
NUM_FILTERS   = 32         
HIDDEN_DROPOUT = 0.0       # dropout after conv+ReLU (0.0 = no dropout)
NUM_EPOCHS    = 1000
BATCH_SIZE    = 256
...
class RelationRankEvaluator:
    ...
    def evaluate(self, mapped_triples, additional_filter_triples=None):
        ...
        with torch.no_grad():
            for i in range(0, len(mapped_triples), self.batch_size):
                ...
                for triple in batch:
                    ...
                    # Score all real relations for this (h, ?, t) query
                    ...
                    # Filtered evaluation: mask other known true relations
        ...
        print(f"\n  Hits@3={h_at_3:.4f} / Hits@10={h_at_10:.4f} are trivially 1.0 "
              f"with only {len(per_rel)} relations — ignore them.")
...
if __name__ == "__main__":
    ...
    # ── Record PyKEEN results to Excel ────────────────────────────────────────
    try:
    ...
    try:
        ...
        print(f"{'Hits@3':<16} {rel_h3:>14.4f}  {hits_at_3:>16.4f}")
        print(f"{'Hits@10':<16} {rel_h10:>14.4f}  {hits_at_10:>16.4f}")
        ...
        record_results(
            ...
            {"mrr": rel_macro, "hits_at_1": rel_macro,
             "hits_at_3": rel_macro, "hits_at_10": rel_macro, "mean_rank": rel_mr},
    ...
    try:
        # ConvKB entity embeddings are real-valued
