...
OUTPUT_DIR   = ROOT / "outputs" / "complex_sup_output"             # folder for saved outputs
...
class RelationRankEvaluator:
    ...
    def evaluate(self, mapped_triples, additional_filter_triples=None):
        ...
        # Build filter map: (h_id, t_id) → set of known true relation IDs
        filter_map = {}
        ...
        with torch.no_grad():
            for i in range(0, len(mapped_triples), self.batch_size):
                ...
                for triple in batch:
                    ...
                    # Filtered eval — mask other known true relations for this (h,t)
...
if __name__ == "__main__":
    ...
    try:
        entity_emb_complex = result.model.entity_representations[0](
            indices=None
        ).detach().cpu()

        if entity_emb_complex.is_complex():
            entity_emb_real = torch.cat(
                [entity_emb_complex.real, entity_emb_complex.imag], dim=1
            ).numpy().astype(np.float32)
        else:
            entity_emb_real = entity_emb_complex.numpy().astype(np.float32)
