"""
For 2-relation training (CONTRARY_TO, NOT_CONTRARY)
...
CSV_PATH     = ROOT / "data" / "input_data" / "hotel_contrary_dataset_all.csv"   # input dataset
#CSV_PATH     = "hotel_contrary_dataset_PNNP.csv"
OUTPUT_DIR   = ROOT / "outputs" / "rotate_output"             # folder for saved outputs
#OUTPUT_DIR   = Path("rotate_pnnp_output") 
...
class RelationRankEvaluator:
    """
    Custom evaluator with relation-corruption training.
...
def stratified_split(df, train_ratio=0.8, val_ratio=0.1, random_seed=42):
    """
    Splits dataset maintaining proportions of both domain AND relation
    in each split. 
    """
...
if __name__ == "__main__":
    ...
    try:
        ...
        # RotatE stores complex tensors: shape (num_entities, embedding_dim)
        ...
        # Separate real and imaginary parts — concatenate as real-valued features
        # Final shape: (num_entities, embedding_dim * 2) — fully real-valued for PCA
        entity_emb_real = np.concatenate([
            entity_emb_np.real,   # (num_entities, embedding_dim)
            entity_emb_np.imag,   # (num_entities, embedding_dim)
